...
新建/删除文件时注意将其包括进项目/从项目移除。相关问题通过 `GFP-OpenWorld.vcxproj` 中 `ItemGroup` 排查。
...
│  ├─ utils
│  │  ├─ bounding_box.hpp   // 包围盒
│  │  ├─ frame_rate_indicator.hpp   // 帧率
│  │  ├─ frustum.hpp                // 截头体
│  │  ├─ gl_utility.hpp             // 包装 glGetError
│  │  ├─ glsl_program.hpp/cpp       // 着色器程序
│  │  ├─ input.hpp                  // 键鼠输入结构体
│  │  ├─ plane.hpp                  // 单个平面（法向、原点距离）
│  │  ├─ transform.hpp/cpp          // 物体的世界方位
│  │  ├─ vertex.hpp   // 单个顶点（坐标、法向、材质坐标（2D））
```
...
然后，整体的作业完成流程应该就是不断写新的 `model-shader-application`，例如相机、光源编辑、碰撞检测、文字渲染、物体交互等，然后最后把所有要用到的 `model-shader` 合并到一个最终的 `application` 中。
